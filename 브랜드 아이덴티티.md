...
브랜드 브리프(업종, 타겟, 키워드 등 : JSON) 하나만 입력하면, LLM API와 이미지 생성 API를 조합하여
**브랜드 네이밍 · 슬로건 · 브랜드 스토리 · 컬러 팔레트 · 로고 시안**까지 자동으로 생성해주는
CLI 기반 Python 프로그램입니다.

*브리프: 프로젝트나 브랜드의 핵심 정보를 담은 요약 문서*
```

브랜드 브리프(JSON) → [LLM API + 이미지 생성 API] → 브랜드 아이덴티티 패키지(텍스트 + 이미지)
...
2. [과제 목표](#2-과제-목표)
3. [기능 요구사항](#3-기능-요구사항)
4. [시스템 아키텍처 / 파이프라인](#4-시스템-아키텍처--파이프라인)
5. [입력 / 출력 스펙](#5-입력--출력-스펙)
6. [기술 스택](#6-기술-스택)
7. [프로젝트 구조](#7-프로젝트-구조)
8. [API 오류 상황과 대응 방법](#8-api-오류-상황과-대응-방법)
9. [프롬프트 엔지니어링 원칙](#9-프롬프트-엔지니어링-원칙)
10. [보너스 과제](#10-보너스-과제)
11. [개발 환경 설정 가이드 (초보자용)](#11-개발-환경-설정-가이드-초보자용)
12. [실행 방법 & 실행 예시](#12-실행-방법--실행-예시)
13. [Git 명령어 정리 (초보자용)](#13-git-명령어-정리-초보자용)
14. [체크리스트](#14-체크리스트)
...
하나하나가 시간과 전문성을 요구하는 전문 작업이기 때문입니다.

이 프로젝트는 그 작업 과정을 **브리프 하나로 자동화**하는 것을 목표로 합니다. 업종·타겟·키워드 등 브랜드 정보를
JSON 파일로 입력받으면, 
LLM API로 텍스트 기반 브랜드 요소(네이밍/슬로건/스토리/컬러)를 생성하고,
이미지 생성 API로 로고 시안까지 만든 뒤 모든 결과물을 지정된 폴더 하나에 JSON과 PNG 형태로 저장합니다.

| 이유 | 설명 |
|---|---|
| 실무 연결성 | 실제 브랜딩 에이전시 업무를 AI로 자동화하는 사례 |
| 멀티모달 경험 | 텍스트 AI(LLM) + 이미지 AI를 하나의 파이프라인에서 함께 다룸 |
| 파이프라인 사고 | 여러 API를 조합하는 시스템 설계를 훈련 |
| 실용성 | 결과물을 실제 포트폴리오/시안으로 바로 활용 가능 |
...
이 과제를 완료하면 아래 4가지를 스스로 설명할 수 있어야 합니다. 각 목표를 이 프로젝트에서
**어떻게 구현했는지** 함께 정리했습니다.
...
| 10 | API 키 관리 | 코드에 키를 직접 작성하지 않고 `.env` 파일 + `python-dotenv`로 환경변수에서 로드 | `utils/llm_client.py`, `.env.example` |
...
```
output/
├── brand_result.json     # 모든 텍스트 결과 + 메타데이터
├── color_palette.png     # 컬러 팔레트 시각화
├── logo_01.png           # 로고 시안 1
└── logo_02.png           # 로고 시안 2
```

### 📄 `brand_result.json` 스키마
...
> 💡 **왜 `gpt-image-1`인가?** 과제 문서에는 예시로 DALL·E가 언급되어 있지만, 개발 시점 기준
> `dall-e-3`는 신규 모델로 대체되는 추세이며 `gpt-image-1`은 별도의 `response_format` 파라미터 없이
> 기본적으로 `b64_json` 형식을 반환해 URL 다운로드 없이 바로 파일로 저장할 수 있다는 실무적 장점이 있습니다.
...
프로젝트 구조뿐 아니라 파이프라인 순서, 사용 모델 등 곳곳에서 여러 선택지 중 하나를 골라야
했습니다. "왜 이렇게 했는가"를 한 곳에 모아 정리했습니다.

| 결정 사항 | 선택한 방식 | 이유 |
|---|---|---|
| 이미지 생성 모델 | `dall-e-3` 대신 `gpt-image-1` 채택 | ① **실무적 이유**: 개발 시점 기준 OpenAI 이미지 생성 라인업이 `gpt-image-1`로 이전되는 추세였고, 실제로 `dall-e-3`로 개발 중 모델 단종(retire) 이슈를 겪어 전환함 <br> ② **코드 단순화**: `dall-e-3`는 `response_format`(`url`/`b64_json`) 지정이 필요하고 `url` 방식이면 `requests.get()` 재다운로드 단계가 추가되지만, `gpt-image-1`은 해당 파라미터 없이 기본 `b64_json`을 반환해 `base64.b64decode()` 한 번으로 PNG 저장이 끝남(실패 지점 감소) <br> ③ **API 키 통합 관리**: 텍스트(`gpt-4o-mini`)와 이미지가 동일한 `OPENAI_API_KEY`로 호출되어 별도 계정 없이 하나의 키로 파이프라인 전체를 관리 가능 |
| 파이프라인 실행 순서 | 텍스트 생성(네이밍→슬로건→스토리→팔레트) 이후 이미지(로고) 생성 | 이미지 API가 상대적으로 비싸고 느리므로, 텍스트(특히 네이밍/컬러)가 먼저 확보된 뒤 그 결과를 로고 프롬프트에 반영해 일관성을 높임 |
| 저장 시점 | 모든 단계 실행 후 마지막에 일괄 저장 | 중간에 일부 단계가 실패해도 성공한 결과만이라도 파일로 남길 수 있도록 함 |
| 네이밍 글자 수 제약 | 프롬프트 지시 + 코드 레벨 하드 트렁케이션(15자) 이중 적용 | 프롬프트만으로는 LLM이 제약을 100% 지키지 않는 경우가 있어, 코드로 안전망을 추가함 ("이중 안전망" 원칙) |
| 에러 처리 단위 | 단계별(naming/slogan/story/palette/logo) 개별 `try-except` | 하나의 API 호출이 실패해도 전체 파이프라인이 중단되지 않고 나머지 단계가 계속 진행되도록 함 |

---

## 8. API 오류 상황과 대응 방법

> 과제 목표 4번 "API 호출 시 발생할 수 있는 오류 상황과 대응 방법을 설명할 수 있다"에 대한 답으로,
...
프롬프트가 뜨면 브리프 파일 경로(예: `brief_example.json`)를 입력하면 됩니다.
...
- [x] LLM API로 네이밍 3~5개 생성 (한글/영문)
- [x] LLM API로 슬로건 3개 생성
- [x] LLM API로 브랜드 스토리 300자 내외 생성
- [x] LLM API로 컬러 팔레트 생성 (메인 + 서브, HEX)
- [x] matplotlib으로 팔레트 PNG 저장
- [x] 이미지 API로 로고 시안 PNG 저장
- [x] 모든 텍스트 결과를 `brand_result.json`으로 저장
- [x] 각 API 호출을 개별 `try-except`로 감쌈
- [x] 환경변수(`.env`)로 API 키 관리
